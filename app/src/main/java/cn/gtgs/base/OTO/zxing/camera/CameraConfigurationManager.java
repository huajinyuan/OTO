...
package cn.gtgs.base.OTO.zxing.camera;
...
final class CameraConfigurationManager {
	...
	void initFromCameraParameters(Camera camera) {
		...
		F.d("Default preview format: " + previewFormat + '/' + previewFormatString);
		...
		// display.getHeight());
		F.d("Screen resolution: " + screenResolution);
		cameraResolution = getCameraResolution(parameters, new Point(width, height));
		F.d("Camera resolution: " + screenResolution);
		/*
		 * Camera.Parameters parameters = camera.getParameters(); previewFormat
		 * = parameters.getPreviewFormat(); previewFormatString =
		 * parameters.get("preview-format"); WindowManager manager =
		 * (WindowManager) context .getSystemService(Context.WINDOW_SERVICE);
		 * Display display = manager.getDefaultDisplay(); int width =
		 * display.getWidth(); int height = display.getHeight(); if (width <
		 * height) { int temp = width; width = height; height = temp; }
		 * screenResolution = new Point(height, width); cameraResolution =
		 * getCameraResolution(parameters, new Point(width, height));
		 */
	...
	private static Point getCameraResolution(Camera.Parameters parameters, Point screenResolution) {
		...
		if (previewSizeValueString != null) {
			F.d("preview-size-values parameter: " + previewSizeValueString);
	...
	private static Point findBestPreviewSizeValue(CharSequence previewSizeValueString, Point screenResolution) {
		...
		for (String previewSize : COMMA_PATTERN.split(previewSizeValueString)) {
			...
			if (dimPosition < 0) {
				F.d("Bad preview-size: " + previewSize);
				continue;
			}
	...
	private void setZoom(Camera.Parameters parameters) {
		...
		if (maxZoomString != null) {
			try {
				...
				F.d("Bad max-zoom: " + maxZoomString);
			}
		}
		...
		if (takingPictureZoomMaxString != null) {
			try {
				...
				F.d( "Bad taking-picture-zoom-max: " + takingPictureZoomMaxString);
			}
		}
