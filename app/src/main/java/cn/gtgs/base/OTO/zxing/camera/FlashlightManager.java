...
final class FlashlightManager {
	...
	static {
		...
		if (iHardwareService == null) {
			F.d("This device does supports control of a flashlight");
		} else {
			F.d("This device does not support control of a flashlight");
		}
	}
	...
	private static Class<?> maybeForName(String name) {
		try {
			...
			F.d("Unexpected error while finding class " + name + re);
			return null;
		}
	}
	...
	private static Method maybeGetMethod(Class<?> clazz, String name, Class<?>... argClasses) {
		try {
			...
			F.d("Unexpected error while finding method " + name + re);
			return null;
		}
	}
	...
	private static Object invoke(Method method, Object instance, Object... args) {
		try {
			...
			F.d("Unexpected error while invoking " + method + e);
			return null;
		} catch (InvocationTargetException e) {
			F.d("Unexpected error while invoking " + method + e.getCause());
			return null;
		} catch (RuntimeException re) {
			F.d("Unexpected error while invoking " + method + re);
			return null;
		}
	}
